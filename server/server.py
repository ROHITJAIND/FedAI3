...
# Initialize global model (text-based neural network)
global_model = create_model(
    input_dim=3,  # Gestational_Age, Baby_Heart_Rate, Amniotic_Fluid_Index
    num_classes=4,  # Cephalic, Breech, Transverse, Not Fixed
...
if model_path.exists():
    try:
        ...
        print(f"[OK] Loaded existing global model from {model_path}")
    except Exception as e:
        print(f"[WARN] Could not load existing model: {e}")
        aggregator.global_weights = global_model.state_dict()
else:
...
def start_server(host: str = None, port: int = None):
    ...
    print("=" * 60)
    print("FEDAI3 CENTRAL SERVER - TEXT-BASED MODEL")
    print("=" * 60)
    print(f"[START] Starting server at http://{host}:{port}")
    print(f"[MODEL] Medical Text Classifier")
    print(f"[INPUT] 3 features (Gestational Age, Heart Rate, AFI)")
    print(f"[OUTPUT] 4 classes (Cephalic, Breech, Transverse, Not Fixed)")
