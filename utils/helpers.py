...
def load_model(model: torch.nn.Module, path: Path) -> torch.nn.Module:
    ...
    try:
        ...
        # Try to load, but skip if architecture doesn't match
        model.load_state_dict(state_dict, strict=False)
        print(f"[OK] Model loaded from {path}")
        
        if isinstance(checkpoint, dict) and 'timestamp' in checkpoint:
            print(f"   Checkpoint timestamp: {checkpoint['timestamp']}")
            
    except Exception as e:
        # If loading fails due to architecture mismatch, skip and use fresh model
        if "Missing key(s)" in str(e) or "Unexpected key(s)" in str(e):
            print(f"[WARN] Checkpoint architecture mismatch (old CNN model detected)")
            print(f"[INFO] Starting with fresh model - old checkpoint will be overwritten after training")
        else:
